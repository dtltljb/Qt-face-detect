...
int ffmpeg_YUV_to_RGB24(void* argv)
{
	//Parameters	
    FILE *src_file =fopen( (DATA_DIR + "sintel_480x272_yuv420p.yuv").c_str(), "rb");
	const int src_w=480,src_h=272;
	AVPixelFormat src_pixfmt=AV_PIX_FMT_YUV420P;

	int src_bpp=av_get_bits_per_pixel(av_pix_fmt_desc_get(src_pixfmt));

    FILE *dst_file = fopen( (DATA_DIR + "sintel_1280x720_rgb24.rgb").c_str(), "wb");
